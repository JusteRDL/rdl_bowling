    ...
    while true do
        ...
        if not bowling then
            ...
            if distance <= 15.0 then
                sleep = 0
                DrawMarker(27, Config['Bowling']['GetBall'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.5, 0.5, 0.4), 238, 245, 39, 0.87)
                if distance <= 1.5 then
                    helpText(Strings['press_start'])
                    if IsControlJustReleased(0, 38) then
                        bowling = true
                        TriggerServerEvent('rdl_bowling:start')
                    end
                end
            end
        end
...
RegisterNetEvent('rdl_bowling:cancel')
AddEventHandler('rdl_bowling:cancel', function(full)
    if full then
...
RegisterNetEvent('rdl_bowling:chooseTrack')
AddEventHandler('rdl_bowling:chooseTrack', function(occupiedTracks)
    ...
    if #tracksLeft >= 1 then
        ...
        while true do
            ...
            DrawMarker(0, current.x, current.y, current.z+1.0, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.5, 0.5, 0.4), 238, 245, 39, 0.87)
            ...
            if IsControlJustReleased(0, 215) then
                for i = 1, #Config['Bowling']['Tracks'] do
                    if Config['Bowling']['Tracks'][i] == tracksLeft[currentTrack] then
                        TriggerServerEvent('rdl_bowling:choose', i)
                    end
                end
                break
...
RegisterNetEvent('rdl_bowling:play')
AddEventHandler('rdl_bowling:play', function(chosenTrack)
    bowling = true 
    DoScreenFadeOut(750)
    ...
    DestroyCam(cam, false)
    ClearPedTasks(me)
    TriggerServerEvent('rdl_bowling:end', chosenTrack)
    for k, v in pairs(pins) do DeleteEntity(v) end
    bowling = false
end)
